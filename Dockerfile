...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get update && apt-get install -y --no-install-recommends \
 ...
 && curl -fsSL https://pkgs.tailscale.com/stable/ubuntu/noble.asc -o /etc/apt/keyrings/tailscale.asc \
 && chmod a+r /etc/apt/keyrings/tailscale.asc \
 && echo "deb [signed-by=/etc/apt/keyrings/tailscale.asc] https://pkgs.tailscale.com/stable/ubuntu noble main" \
